...
class InMemoryDb {
  ...
  private deduplicate() {
    ...
    this.frames = this.frames.filter(f => {
      if (!f || !f.image_url) return false;
      const key = f.image_url.startsWith("data:") ? f.image_url : path.basename(f.image_url);
    ...
    this.stickers = this.stickers.filter(s => {
      if (!s || !s.image_url) return false;
      const key = s.image_url.startsWith("data:") ? s.image_url : path.basename(s.image_url);
      if (seenStickers.has(key)) return false;
  ...
  try {
    ...
    if (fs.existsSync(stickersDir)) {
      ...
      for (const file of files) {
        ...
        const alreadyExists = allStickers.some((s: any) => s.image_url === imageUrl || (s.image_url && s.image_url.includes(file)));
        if (!alreadyExists) {
          let cleanName = file.replace(/\.[^/.]+$/, "").replace(/^sticker-/, "").replace(/[-_]/g, " ");
          if (cleanName === "1") cleanName = "Stiker Heart";
          else if (cleanName === "2") cleanName = "Stiker Star";
          else if (cleanName === "3") cleanName = "Stiker Corner Pattern";
          else if (cleanName === "4") cleanName = "Stiker Sparkle";
          else if (cleanName === "5") cleanName = "Stiker Cute Icon";
          else if (cleanName === "6") cleanName = "Stiker Bow";
          else cleanName = cleanName.charAt(0).toUpperCase() + cleanName.slice(1);

          db.prepare("INSERT INTO stickers (name, image_url) VALUES (?, ?)").run(cleanName, imageUrl);
        }
      }
    }
    ...
    if (fs.existsSync(framesDir)) {
      ...
      for (const file of files) {
        ...
        const alreadyExists = allFrames.some((f: any) => f.image_url === imageUrl || (f.image_url && f.image_url.includes(file)));
        if (!alreadyExists) {
          let cleanName = file.replace(/\.[^/.]+$/, "").replace(/^default-frame-/, "").replace(/[-_]/g, " ");
          if (cleanName === "1") cleanName = "Frame Ribbon Blue";
          else if (cleanName === "2") cleanName = "Frame Floral Soft";
          else if (cleanName === "3") cleanName = "Frame Classic Pattern";
          else if (cleanName === "4") cleanName = "Frame Cute Pastel";
          else cleanName = cleanName.charAt(0).toUpperCase() + cleanName.slice(1);

          db.prepare("INSERT INTO frames (name, image_url, photos_count) VALUES (?, ?, ?)").run(cleanName, imageUrl, 4);
        }
      }
    }
  } catch (err) {
